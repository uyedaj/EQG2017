...
dumdat <- dataSim(truepars, "OU", tree)$dat
cache <- bayou:::.prepare.ou.univariate(tree, dumdat, SE=0, pred=pred)
W <- bayou:::C_weightmatrix(cache, truepars)
set.seed(1)
dat2 <- dataSim(truepars, tree, model="OU")$dat + truepars$beta_lnMass[bayou:::.tipregime(truepars, tree)] * pred[,1]
set.seed(1)
dat <- dataSim(truepars, tree, model="OU")$dat + W$W%*%truepars$beta_lnMass * pred[,1]
dat <- setNames(dat[,1], cache$phy$tip.label)
set.seed(1)
trueparsPGLS <- list(alpha=truepars$alpha, sig2=truepars$sig2, k=0, ntheta=1, theta=0, sb=numeric(0), loc=numeric(0), t2=numeric(0))
dat3 <- dataSim(trueparsPGLS, tree, model="OU")$dat
dat3 <-truepars$theta[bayou:::.tipregime(truepars, tree)] + truepars$beta_lnMass[bayou:::.tipregime(truepars, tree)] * pred[,1] + dat3
```
...
plot(pred[,1], dat2, pch=22, bg=bayou:::.tipregime(truepars, tree), xlab="lnMass", "ylab"="Trait")
dum <- lapply(1:truepars$ntheta, function(x) abline(truepars$theta[x], truepars$beta_lnMass[x],  lty=2, col=x))

plot(pred[,1], dat3, pch=22, bg=bayou:::.tipregime(truepars, tree), xlab="lnMass", "ylab"="Trait")
dum <- lapply(1:truepars$ntheta, function(x) abline(truepars$theta[x], truepars$beta_lnMass[x],  lty=2, col=x))
...
prior.11 <- make.prior(tree, plot.prior = FALSE, 
                       ...
                       param=list(dalpha=list(scale=0.1), dsig2=list(scale=0.1),
                                  dbeta_lnMass=list(mean=0.7, sd=0.2),
                                  dtheta=list(mean=0, sd=1.5)),
...
prior.N1 <- make.prior(tree, plot.prior = FALSE, 
                       ...
                       param=list(dbeta_lnMass=list(mean=0.7, sd=0.2),
...
model.11 <- makeBayouModel(dat ~ lnMass, rjpars = c(), 
                           tree=tree, dat=dat, pred=pred, SE=MEvar, prior=prior.11, D=D11, slopechange="fullPGLS")
model.N1 <- makeBayouModel(dat ~ lnMass, rjpars = c("theta"),  
                           tree=tree, dat=dat, pred=pred, SE=MEvar, prior=prior.N1, D=DN1, slopechange="fullPGLS")
model.NN <- makeBayouModel(dat ~ lnMass, rjpars = c("theta", "lnMass"),  
                           tree=tree, dat=dat, pred=pred, SE=MEvar, prior=prior.NN, D=DNN, slopechange="fullPGLS")
...
## Make MCMC objects:
mcmc.11 <- bayou.makeMCMC(tree, dat, pred=pred, SE=MEvar, model=model.11$model, prior=prior.11, startpar=model.11$startpar, new.dir="../output/AllometryTest/", outname="model11_r006", plot.freq=NULL)
mcmc.N1 <- bayou.makeMCMC(tree, dat, pred=pred, SE=MEvar, model=model.N1$model, prior=prior.N1, startpar=model.N1$startpar, new.dir="../output/AllometryTest/", outname="modelN1_r006", plot.freq=NULL)
mcmc.NN <- bayou.makeMCMC(tree, dat, pred=pred, SE=MEvar, model=model.NN$model, prior=prior.NN, startpar=model.NN$startpar, new.dir="../output/AllometryTest/", outname="modelNN_r006", plot.freq=NULL)
...
shiftsumsN1 <- shiftSummaries(chain.N1, mcmc.N1, pp.cutoff=0.3, burnin=0.3)
shiftsumsNN <- shiftSummaries(chain.NN, mcmc.NN, pp.cutoff=0.3, burnin=0.3)
...
Bk <- qbeta(seq(0,1, length.out=5), 0.3,1)
set.seed(1)
ss.11 <- mcmc.11$steppingstone(20000, chain.11, Bk, burnin=0.3, plot=FALSE)
ss.N1 <- mcmc.N1$steppingstone(20000, chain.N1, Bk, burnin=0.3, plot=FALSE)
ss.NN <- mcmc.NN$steppingstone(20000, chain.NN, Bk, burnin=0.3, plot=FALSE)
